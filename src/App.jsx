...
import logoUrl from "../ganpat-logo.png";
...
function LogoMark({ large = false }) {
  return (
    <span className={large ? "portal-logo-mark" : "brand-mark"}>
      <img src={logoUrl} alt="Ganpat University Logo" style={{ width: '100%', height: '100%', objectFit: 'contain', background: '#fff', borderRadius: '8px', border: large ? '4px solid #fff' : '1px solid #dce4ee', boxShadow: large ? '0 2px 12px rgba(16,42,67,0.08)' : undefined }} />
    </span>
  );
}
...
function Brand({ footer = false }) {
  return (
    <a className={`brand ${footer ? "footer-brand" : ""}`} href="#home" aria-label="Ganpat University demo home">
      <LogoMark />
      <span>
        <strong>Ganpat University</strong>
        <small>Demo Portal</small>
      </span>
    </a>
  );
}
    ...
    <main id="home">
      ...
      <section id="terms" className="section terms-section">
        <div className="section-heading compact">
          ...
          <h2>Demo portal usage policy</h2>
        </div>
        ...
        <section className="portal-form-pane">
          ...
          <form className={`auth-pane ${authMode === "signup" ? "active" : ""}`} aria-label="Student sign up form">
            ...
            <label className="check-row portal-check">
              <input type="checkbox" required />
              <span>
                I agree to the Terms & Conditions and Privacy Policy, and authorize Ganpat University to contact me by
